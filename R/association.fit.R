        ...
        if (genotypingRate >= controlGeno) {
          ...
          if (length(table(as.character(var))) == 3) {
            ...
            if (is.null(adj)) {
              ...
              if (any(type %in% 6) | any(type %in% 2)) {
                m.dom <- glm(dep ~ var.dom,
                  subset = subset,
                  family = binomial, ...
                )
              }
              if (any(type %in% 6) | any(type %in% 3)) {
                m.rec <- glm(dep ~ var.rec,
                  subset = subset,
                  family = binomial, ...
                )
              }
              if (any(type %in% 6) | any(type %in% 4)) {
                m.over <- glm(dep ~ var.over,
                  subset = subset,
                  family = binomial, ...
                )
              }
              if (any(type %in% 6) | any(type %in% 5)) {
                m.ad <- glm(dep ~ as.numeric(var.co),
                  subset = subset,
                  family = binomial, ...
                )
              }
            ...
            if (any(type %in% 6) | any(type %in% 5)) {
              ...
              Ad <- c(ansTot, intervals.or(
                m.ad,
                level, m.b
              )$or.ic, round(AIC(m.ad), 1))
              if (any(temptp == 0) & is.null(adj)) {
                pp <- fisher.test(dep, var.co)$p
                Ad[8] <- pp
              }
            ...
            co <- cbind(
              Table.N.Per(var.co, dep, subset)$tp,
        ...
        if (length(table(as.character(var))) == 3) {
          ...
          if (any(type %in% 6) | any(type %in% 1)) {
            co <- cbind(Table.mean.se(var.co, dep, subset)$tp,
              intervals.dif(m.co, level, m.b, var)$m,
              AIC = c(round(
                AIC(m.co),
                1
              ), NA, NA)
          ...
          co <- cbind(Table.mean.se(var.co, dep, subset)$tp,
            intervals.dif(m.co, level, m.b, var.co)$m,
            AIC = c(
              AIC(m.co),
              NA
            )
