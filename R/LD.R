...
LD.snp <-
  function(g1, g2, ...) {
    ...
    if (pA < 1 & pB < 1) {
      ...
      loglik <- function(pAB, ...) {
        (2 * n3x3[1, 1] + n3x3[1, 2] + n3x3[2, 1]) * log(pAB) +
          (2 * n3x3[1, 3] + n3x3[1, 2] + n3x3[2, 3]) * log(pA -
            pAB) + (2 * n3x3[3, 1] + n3x3[2, 1] + n3x3[
            3,
            2
          ]) * log(pB - pAB) + (2 * n3x3[3, 3] + n3x3[3, 2] +
            n3x3[2, 3]) * log(1 - pA - pB + pAB) + n3x3[2, 2] *
            log(pAB * (1 - pA - pB + pAB) + (pA - pAB) * (pB -
              pAB))
      }
      solution <- optimize(loglik,
      ...
      retval <- list(
        call = match.call(), D = estD, "D'" = estDp,
        r = corr, "R^2" = corr^2, n = n, "X^2" = dchi, "P-value" = dpval
      )
    } else {
      retval <- list(
        call = match.call(), D = NA, "D'" = NA,
...
LDtable <-
  function(
    x, colorcut = c(0, 0.01, 0.025, 0.05, 0.1, 1), colors = heat.colors(length(colorcut)),
    textcol = "black", digits = 3, show.all = FALSE, which = c(
      "D",
      "D'", "r", "X^2", "P-value", "n"
    ), colorize = "P-value",
    cex, ...
    ...
    image(
      x = 1:ncol(colormat), y = 1:ncol(colormat), z = t(colormat[nrow(colormat):1, ]), col = colors, xlab = "Marker 2\n\n", ylab = "Marker 1",
      xaxt = "n", yaxt = "n", ...
    )
    ...
    if (missing(cex)) {
      cex <- min(c(1 / 10, 1 / (length(which) + 1)) / c(
        strwidth("W"),
        strheight("W") * 1.5
      ))
    }
    ...
    for (i in 1:length(which)) {
      ...
      text(
        x = col(colormat), y = nrow(colormat) - row(colormat) +
...
LDplot <-
  function(x, digits = 3, marker, distance, which = c(
             "D", "D'",
             "r", "X^2", "P-value", "n", " "
           ), ...) {
