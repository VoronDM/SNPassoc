...
haplo.score <- function(y, geno, trait.type = "gaussian",
  ...
  # Choose trait type:
  trait.int <- charmatch(trait.type, c(
  ...
  haplo <- haplo.em(geno, locus.label,
    miss.val = miss.val,
    control = em.control
  )
  ...
  if (trait.int == 4) {
    if (adjusted) {
      ...
      tmp <- haplo.score.podds(
        y, alpha, beta, x.adj, nreps, x.post,
        post, x
      )
    }
    ...
    if (!adjusted) {
      ...
      alpha <- -log((1 - s[-1]) / s[-1])
      tmp <- haplo.score.podds(y, alpha,
        beta = NA, x.adj = NA, nreps, x.post,
        post, x
      )
    }
  ...
  if (!simulate) {
    ...
    while (!done) {
      ...
      if (trait.int == 4) {
        if (adjusted) {
          tmp <- haplo.score.podds(
            y[rand.ord], alpha, beta,
            x.adj[rand.ord, , drop = FALSE], nreps, x.post, post, x
          )
        }

        if (!adjusted) {
          tmp <- haplo.score.podds(y[rand.ord], alpha,
            beta = NA,
  ...
  obj <- (list(
    score.global = score.global, df = df, score.global.p = score.global.p,
