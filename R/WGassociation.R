`WGassociation` <-
  function(formula, data, model = c("all"), quantitative = is.quantitative(
             formula,
             data
    ...
    association.i <- function(snp.i, adj, data, model, quantitative, genotypingRate, level, ...) {
      association(as.formula(paste(adj, "+", snp.i)),
        data = data,
    ...
    tab <- mclapply(SNPs, association.i,
      adj = adj, data = data, model = model,
