`interactionPval` <-
  function(formula, data, quantitative = is.quantitative(
             formula,
             data
           ), model = "codominant") {
    ...
    if (is.vector(colSNPs) & length(colSNPs) > 0) {
      ...
      stop("data should have an attribute called 'colSNPs'. Try again 'setupsNP' function")
    }
      ...
      if (is.Monomorphic(dataSNPs[, i])) {
        ...
        while (j <= n) {
          ...
          if (is.Monomorphic(dataSNPs[, j])) {
            ...
            mod.b1 <- glm(as.formula(paste(adj, "+ dataSNPs[, i]")),
              data = data,
              family = fam, subset = subset
            )
            mod.b2 <- glm(as.formula(paste(adj, "+ dataSNPs[, j]")),
              data = data,
              family = fam, subset = subset
            )
        ...
        mod.0 <- glm(as.formula(paste(adj, "+ dataSNPs[, i]")),
          data = data,
          family = "gaussian"
        )
        subset <- 1:nrow(data) %in% as.numeric(rownames(mod.0$model))
        mod.b <- glm(as.formula(paste(adj)),
          data = data,
