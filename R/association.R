`association` <-
  function(
    ...
    subset, name.snp = NULL, quantitative = is.quantitative(
      formula,
      data
    ), genotypingRate = 0, level = 0.95, ...
  ) {
    cl <- match.call()
    ...
    if (any(ord > 1)) {
      ...
      res.int2 <- ans2
      res <- list(
        res.corner, res.int1, res.int2, p.interaction,
        p.trend1, p.trend2
      )
      interaction <- TRUE
    } else {
      type <- charmatch(model, c(
        "codominant", "dominant",
      ...
      if (length(strats)) {
        ...
        if (length(temp$vars) == 1) {
          ...
          strata.keep <- strata(mf[, temp$vars], shortlabel = TRUE)
        }
