  ...
  function(
    ...
    image(1:nrow(data.Missing), 1:ncol(data.Missing), data.Missing,
      col = c("white", "black"), ylab = "Individuals", xlab = ifelse(print.labels.SNPs,
        "", "SNPs"
      ), axes = !print.labels.SNPs
    )
    if (print.labels.SNPs) {
      axis(1,
        at = c(1:length(label.SNPs)), labels = label.SNPs,
        las = 3, cex.axis = 0.7
      )
      axis(2)
    }
    ...
    if (!is.null(genInfo)) {
      ...
      col.ok <- c("black", rep("black", length(a) - 1))
    }
    ...
    if (!is.null(genInfo)) {
      segments(a, b[3], a, b[4] + diff(b[3:4]) * 0.02, col = c(
        "black",
        rep("red", length(a) - 1)
      ))
      ...
      for (i in 1:n.gen) {
        text(mean(wh[i + c(0, 1)]), a[4] +
          (a[4] - a[3]) * 0.025, names.geno[i],
        srt = 45, cex = 0.8,
        adj = 0.2
        )
      }
