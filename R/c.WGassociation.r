...
c.WGassociation <-
  function(...) {
    ...
    lapply(1:n, function(i) {
      if (!inherits(allargs[[i]], "WGassociation")) {
    ...
    for (i in 2:n) {
      ...
      attr(out, "tables") <- c(attr(x1, "tables"), attr(
        xi,
        "tables"
      ))
      attr(out, "label.SNPs") <- c(
        attr(x1, "label.SNPs"),
        attr(xi, "label.SNPs")
      )
      attr(out, "colSNPs") <- c(attr(x1, "colSNPs"), attr(
        xi,
        "colSNPs"
      ))
      attr(out, "gen.info") <- rbind(
        attr(x1, "gen.info"),
        attr(xi, "gen.info")
      )
