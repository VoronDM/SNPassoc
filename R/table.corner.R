  ...
  function(var, dep, adj = NULL, int = NULL, num.status, level) {
    glm2 <-
      function(
        ...
        fit <- glm.fit(
          ...
          family = family, control = control, intercept = attr(
            mt,
            "intercept"
          ) > 0
        )
        if (length(offset) && attr(mt, "intercept") > 0) {
          fit$null.deviance <- glm.fit(
        ...
        fit <- c(fit, list(
          ...
          contrasts = attr(X, "contrasts"), xlevels = .getXlevels(
            mt,
            mf
          )
        ))
    ...
    if (num.status == 0) {
      ...
      var.int <- factor(paste(levels(var)[var], levels(int)[int]),
        ...
        exclude = c(
          paste(levels(var), ""), paste("", levels(int)),
          paste(" ")
        )
      )
      if (is.null(adj)) {
      ...
      res <- cbind(table(var.int[subset], dep[subset]), intervals.or(
        m.var.int,
        level, m.b, var.int
      )$or.ic)[, 1:5]
      i <- 1
      ...
      res <- cbind(
        Table.mean.se(var.int, dep, subset)$tp,
